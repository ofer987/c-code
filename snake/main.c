...
size_t screen_background_colors[TOTAL] = {
  [AVAILABLE] = TB_DEFAULT,          [USED_BY_SNAKE_TAIL] = TB_BLACK,
  [USED_BY_SNAKE_HEAD] = TB_BLACK,   [USED_BY_FOOD] = TB_BLACK,
  [USED_BY_TOP_BORDER] = TB_BLUE,    [USED_BY_TOP_RIGHT_BORDER] = TB_BLUE,
  [USED_BY_RIGHT_BORDER] = TB_BLUE,  [USED_BY_BOTTOM_RIGHT_BORDER] = TB_BLUE,
  [USED_BY_BOTTOM_BORDER] = TB_BLUE, [USED_BY_BOTTOM_LEFT_BORDER] = TB_BLUE,
  [USED_BY_LEFT_BORDER] = TB_BLUE,   [USED_BY_TOP_LEFT_BORDER] = TB_BLUE,
...
void init_screen(enum screen_state* screen_states, struct snake_struct* snake, struct coordinates* food_location);

void eat_food(struct snake_struct* snake, struct coordinates* food_location);
bool is_same_coordinates(struct coordinates* coord_01, struct coordinates* coord_02);
void render_screen(enum screen_state* screen_states, struct results_struct* results);
void generate_food(enum screen_state* screen_states, struct coordinates* food_location);
...
bool is_game_valid(enum screen_state* screen_states, struct coordinates* snake_head);
struct coordinates index_to_coorindates(size_t index);
size_t coordinates_to_index(size_t x, size_t y);
struct coordinates* add_head(struct snake_struct* snake, struct coordinates* head);
...
void
show_results(struct results_struct* results) {
  tb_printf(MESSAGE_BOARD_START_COLUMN + 3, MESSAGE_BOARD_START_ROW + 8, TB_WHITE, TB_DEFAULT, "The snake was %zu",
            results->result);
  tb_printf(MESSAGE_BOARD_START_COLUMN + 3, MESSAGE_BOARD_START_ROW + 9, TB_WHITE, TB_DEFAULT, "Minutes: %zu",
            results->minutes);
  tb_printf(MESSAGE_BOARD_START_COLUMN + 3, MESSAGE_BOARD_START_ROW + 10, TB_WHITE, TB_DEFAULT, "Seconds: %zu",
            results->seconds);
}

void
draw_top_border(enum screen_state* screen_states) {
  size_t x = 0;
  size_t y = 0;
...
void
draw_right_border(enum screen_state* screen_states) {
...
void
draw_bottom_border(enum screen_state* screen_states) {
  size_t x = BORDER_HORIZONTAL_SIZE - 1;
...
void
draw_left_border(enum screen_state* screen_states) {
  size_t x = 0;
  size_t y = BORDER_VERTICAL_SIZE - 1;
  size_t i = coordinates_to_index(x, y);
...
main(int argc, char* argv[]) {
  ...
  struct snake_struct snake = {.head = &head, .list_size = 1, .current_movement = RIGHT, .quit = false};
  ...
  tinfo[0].snake = &snake;
  s = pthread_create(&keyboard_thread_id, &attr, &keyboard_input, &tinfo[0]);
  if (s != 0) {
  ...
  for (;;) {
    switch (snake.current_movement) {
      case UP:
        // FALLTHROUGH
      case DOWN: usleep(115000); break;
      case LEFT:
        // FALLTHROUGH
      case RIGHT: usleep(100000); break;
    }
...
static void
finish(int sig, struct snake_struct* snake, time_t start_time) {
  time_t end_time = time(NULL);
  is_finished = true;
...
void
init_screen(enum screen_state* screen_states, struct snake_struct* snake, struct coordinates* food_location) {
  draw_top_border(screen_states);
  draw_right_border(screen_states);
  draw_bottom_border(screen_states);
  draw_left_border(screen_states);
...
void
render_screen(enum screen_state* screen_states, struct results_struct* results) {
...
void
generate_food(enum screen_state* screen_states, struct coordinates* food_location) {
...
struct coordinates
new_head_coordinates(struct snake_struct* snake) {
  struct coordinates new_head = {.state = USED_BY_SNAKE_HEAD};
...
void
move_snake(struct coordinates* new_head, struct snake_struct* snake) {
...
bool
is_game_valid(enum screen_state* screen_states, struct coordinates* snake_head) {
  ...
  size_t snake_head_location = coordinates_to_index(snake_head->x, snake_head->y);
  ...
  switch (snake_location) {
    ...
    case USED_BY_TOP_LEFT_BORDER: /* FALLTHROUGH */ return false;
    default: return true;
  }
}
...
void
eat_food(struct snake_struct* snake, struct coordinates* food_location) {
...
bool
is_same_coordinates(struct coordinates* coord_01, struct coordinates* coord_02) {
  return coord_01->x == coord_02->x && coord_01->y == coord_02->y;
}

struct coordinates*
add_head(struct snake_struct* snake, struct coordinates* head) {
...
render_the_message_box_boarders() {
  ...
  // TOP-RIGHT Corner
  tb_printf(MESSAGE_BOARD_START_COLUMN + MESSAGE_BOARD_WIDTH, MESSAGE_BOARD_START_ROW, color, TB_DEFAULT,
            BORDER_TOP_RIGHT);

  // BOTTOM-LEFT Corner
  tb_printf(MESSAGE_BOARD_START_COLUMN, MESSAGE_BOARD_START_ROW + MESSAGE_BOARD_HEIGHT, color, TB_DEFAULT,
            BORDER_BOTTOM_LEFT);

  // BOTTOM-RIGHT Corner
  tb_printf(MESSAGE_BOARD_START_COLUMN + MESSAGE_BOARD_WIDTH, MESSAGE_BOARD_START_ROW + MESSAGE_BOARD_HEIGHT, color,
            TB_DEFAULT, BORDER_BOTTOM_RIGHT);
}

static void
turn_left(struct snake_struct* snake) {
...
static void
turn_right(struct snake_struct* snake) {
  if (snake->current_movement != LEFT) {
...
static void
turn_up(struct snake_struct* snake) {
...
static void
turn_down(struct snake_struct* snake) {
...
static void
reinit_snake(struct snake_struct* snake) {
  snake->pause = false;
  snake->quit = false;
}
...
keyboard_input(void* arg) {
  ...
  while (!is_finished) {
    ...
    char ch = ev.ch;
    switch (ch) {
      case 'p': snake->pause = true; break;
      case 'q': snake->quit = true; break;
      case 'h': turn_left(snake); break;
      case 'k': turn_up(snake); break;
      case 'l': turn_right(snake); break;
      case 'j': turn_down(snake); break;
    }

    switch (ev.key) {
      case TB_KEY_ARROW_LEFT: turn_left(snake); break;
      case TB_KEY_ARROW_UP: turn_up(snake); break;
      case TB_KEY_ARROW_RIGHT: turn_right(snake); break;
      case TB_KEY_ARROW_DOWN: turn_down(snake); break;
    }
  }
